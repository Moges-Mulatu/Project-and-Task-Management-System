...
const Dashboard = () => {
  ...
  const recentTasks = [
    { id: 1, title: 'Design new landing page', status: 'In Progress', priority: 'High', project: 'Website Redesign' },
    { id: 2, title: 'Fix navigation bug', status: 'Todo', priority: 'Medium', project: 'Bug Fixes' },
    { id: 3, title: 'Update documentation', status: 'In Progress', priority: 'Low', project: 'Documentation' },
    { id: 4, title: 'Code review for PR #234', status: 'Todo', priority: 'High', project: 'Development' },
  ];
              ...
              {recentTasks.map((task) => (
                <div key={task.id} className="flex items-center justify-between p-4 bg-card-background/50 rounded-lg border border-card-border/50 hover:bg-card-background transition-colors duration-200">
                  ...
                  <div className="text-right ml-4">
                    ...
                    <p className="text-xs text-text-muted mt-2">{task.status}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <Card.Body>
            <div className="space-y-3">
              {permissions?.canCreateProjects && (
                <button className="w-full text-left px-4 py-3 bg-gradient-to-r from-brand-blue/10 to-brand-blue/20 text-brand-blue border border-brand-blue/30 rounded-lg hover:from-brand-blue/20 hover:to-brand-blue/30 transition-all duration-200 group">
                  <div className="flex items-center justify-between">
              ...
              {permissions?.canCreateTasks && (
                <button className="w-full text-left px-4 py-3 bg-gradient-to-r from-brand-green/10 to-brand-green/20 text-brand-green border border-brand-green/30 rounded-lg hover:from-brand-green/20 hover:to-brand-green/30 transition-all duration-200 group">
                  <div className="flex items-center justify-between">
