...
const TeamsScreen = ({ navigation, user }) => {
  ...
  const loadTeams = useCallback(async () => {
    ...
    try {
      ...
      let teamsList = response.data || [];

      if (user?.role === "team_member") {
        const memberTeamIds = await getMemberTeamIds(teamsList);
        teamsList = teamsList.filter((team) => memberTeamIds.includes(team.id));
      }
      ...
      >
        ...
        <View style={styles.membersSection}>
          ...
          <View style={styles.avatarStack}>
            ...
            {[0, 1, 2, 3]
              .slice(0, Math.min(4, item.currentMemberCount || 1))
              .map((i) => (
                <View
                  key={i}
                  style={[
