  ...
  static async getTasks(filters = {}) {
    try {
      ...
      if (filters.userRole === "team_member" && filters.requesterId) {
        queryOptions.assignedToUserId =
          filters.assignedToUserId || filters.requesterId;
        queryOptions.assignedToTeamIds = filters.assignedToTeamIds || [];
      }

      // If user is project_manager, restrict to tasks in projects they manage
      if (filters.userRole === "project_manager" && filters.projectManagerId) {
        queryOptions.projectManagerId = filters.projectManagerId;
      }
  ...
  static async updateTask(id, updateData, requesterId, requesterRole) {
    try {
      ...
      if (requesterRole === "team_member") {
        const isDirectAssignment = task.assignedTo === requesterId;
        const isUserAssignment =
          Array.isArray(task.assignedToUsers) &&
          task.assignedToUsers.includes(requesterId);

        let isTeamAssignment = false;
        if (
          Array.isArray(task.assignedToTeams) &&
          task.assignedToTeams.length > 0
        ) {
          ...
          isTeamAssignment = task.assignedToTeams.some((teamId) =>
            teamIds.includes(teamId),
          );
        }

        if (!isDirectAssignment && !isUserAssignment && !isTeamAssignment) {
